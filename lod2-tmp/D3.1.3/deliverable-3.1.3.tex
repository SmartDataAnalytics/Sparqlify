...
The query was chosen as it demonstrates a case where a quad \texttt{(:g ?s ?t ?s)} \todo{add :g here, such that it is a quad} implicitly constrains its own terms (the subject and object position need to be equal)
...
\begin{itemize}
\item $\mathit{keys}(b, v) := \left\{ k | \exists k: v \in b(k) \right\}$ is the function that for a given variable (or constant) returns all the binding's keys that map to a set containing it.
\item $\mathit{Keys}(b, V) := \bigcup_{v \in V} \left \{ keys(b, v) \right \}$ is similar to above, however it takes a set of variables as the argument.
\item $\mathit{Values}(b, K) := \bigcup_{v \in V} \left \{ b(v) \right \}$ returns the union of the value-sets for a set of keys.
\end{itemize}
...
with the goal of a mapping language in this spirit eventually becoming an alternative to R2RML.

...
\begin{itemize}
  ...
  \item If the query contained filters, we can infer constrainst as well.\todo{State explicitly: We support inequalities, prefix and type constraints} %such as \{ ?g ?s ?p ?o . Filter(?s = ?o \&\& ?o = rdfs:Class) \}
...
%Answering a query on a single view that only defines variable free quads is equivalent to answering a query on an ordinary triple store.
%Conversely, answering a query on a set of views can be seen as a generalization of answering a query on a triple store.
%This fact suggests, that traditional relational indexing and query planning techniques can be reused for candidate view selection.
%\todo{No! The thing is, as soon as we try out a binding, the set of constraints changes, and therefore affects the next lookup.}
...
\end{tabular}
\caption{Example of the result set for the query Select * \{ ?s ?p ?o \} on the given views}
\end{figure}

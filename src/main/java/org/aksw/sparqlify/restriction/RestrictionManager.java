...
public class RestrictionManager implements IRestrictionManager {
	...
	private RestrictionManager parent;
	
	//private IndirectEquiMap<Var, Restriction> restrictions = new IndirectEquiMap<Var, Restriction>();
	private HashMap<Var, Restriction> restrictions = new HashMap<Var, Restriction>();
	...
	public RestrictionManager(RestrictionManager parent) {
		this.parent = parent;
		this.cnf = new NestedNormalForm(parent.getCnf(), true);
	}
	
	public RestrictionManager(NestedNormalForm cnf) {
		this.cnf = cnf;
	...
	*/
	...
	public Set<Var> getVariables() {
		...
		RestrictionManager current = this;
		while(current != null) {
	...
	public boolean stateRestriction(RestrictionManager rm) {
		...
		for(Var var : vars) {
			Restriction r = getRestriction(var);
			if(r != null) {

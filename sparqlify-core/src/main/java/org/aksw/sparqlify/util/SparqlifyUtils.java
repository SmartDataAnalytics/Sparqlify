...
import org.aksw.sparqlify.core.algorithms.SqlOpSerializerOracle;
import org.aksw.sparqlify.core.algorithms.TypeSerializer;
import org.aksw.sparqlify.core.algorithms.TypeSerializerOracle;
...
import org.aksw.sparqlify.core.cast.SqlLiteralMapperPostgres;
...
//
    public static ViewDefinitionFactory createViewDefinitionFactory(Connection conn, TypeSystem datatypeSystem, Map<String, String> typeAlias) throws IOException {
        ...
        SchemaProvider schemaProvider = new SchemaProviderOracle(conn, datatypeSystem);
    ...
    public static Set<String> getTableNames(Iterable<ViewDefinition> viewDefinitions) {
        Set<String> result = new HashSet<String>();
        for(ViewDefinition vd : viewDefinitions) {
            SqlOp sqlOp = vd.getMapping().getSqlOp();
            if(sqlOp instanceof SqlOpTable) {
                SqlOpTable sqlOpTable = (SqlOpTable)sqlOp;
                result.add(sqlOpTable.getTableName());
            }
        }
    ...
    public static SparqlSqlStringRewriterImpl createDefaultSparqlSqlStringRewriter(DataSource dataSource, Config config, Long maxResultSetSize, Integer maxQueryExecutionTime) throws SQLException, IOException {
        ...
        try {
            ...
            // TODO If the database is big and only a small portion is mapped, fetching the whole schema is overkill
...
//            databaseSchema = Schema.create(conn, null, tableNames);
        ...
        SqlOpSerializer sqlOpSerializer = new SqlOpSerializerOracle(serializerSystem);
...
//	}
    ...
    public static SparqlSqlOpRewriter createSqlOpRewriter(CandidateViewSelector<ViewDefinition> candidateViewSelector, OpMappingRewriter opMappingRewriter, TypeSystem datatypeSystem, Schema databaseSchema) throws SQLException, IOException {
        ...
        TypeSerializer typeSerializer = new TypeSerializerOracle();
        SqlLiteralMapper sqlLiteralMapper = new SqlLiteralMapperPostgres(typeSerializer);

...
//		result.setAliasName(aliasName);
    ...
    public static SqlOpSelectBlock makeSelect(SqlOpSlice op, Set<String> refs) {
        SqlOpSelectBlock result = makeSelectOracle(op, refs);
        return result;
    }

    public static SqlOpSelectBlock makeSelectDefault(SqlOpSlice op, Set<String> refs) {
        SqlOp newOp = _makeSelect(op.getSubOp(), refs);
        SqlOpSelectBlock result = requireSelectBlock(newOp);
    ...
    public static SqlOpSelectBlock makeSelectOracle(SqlOpSlice op, Set<String> refs) {
        SqlOp newOp = _makeSelect(op.getSubOp(), refs);
        SqlOpSelectBlock result = requireSelectBlock(newOp);

        if(op.getLimit() != null) {
            Long end = op.getLimit();

            if(op.getOffset() != null) {
                end += op.getOffset() + 1;
            }
            result = forceSelectBlock(result);

            result.getConditions().add(new S_Function(TypeToken.Int, "lessThanOrEqual@int", Arrays.<SqlExpr>asList(new S_ColumnRef(TypeToken.Int, "ROWNUM", true), new S_Constant(new SqlValue(TypeToken.Int, end)))));
        }

        if(op.getOffset() != null && op.getOffset() != 0) {
            result.getProjection().put("rnum", new S_ColumnRef(TypeToken.Int, "ROWNUM", true));
            result = forceSelectBlock(result);

            //result.getConditions().add(new S_GreaterThan(new S_ColumnRef(TypeToken.Int, "ROWNUM"), new S_Constant(new SqlValue(TypeToken.Int, op.getOffset() + 1))));
            result.getConditions().add(new S_Function(TypeToken.Int, "greaterThanOrEqual@int", Arrays.<SqlExpr>asList(new S_ColumnRef(TypeToken.Int, "rnum", false), new S_Constant(new SqlValue(TypeToken.Int, op.getOffset() + 1)))));

            // NOTE Because of the pseudocolumn ROWNUM we need to create yet another sub select
            // otherwise an outer SqlOpProject will remove it again
            //result = forceSelectBlock(result);
        }

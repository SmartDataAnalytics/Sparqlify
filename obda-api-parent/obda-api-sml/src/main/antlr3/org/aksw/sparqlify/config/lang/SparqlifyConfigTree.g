...
@header {
    ...
    import org.aksw.obda.domain.api.*;
    import org.aksw.obda.domain.impl.*;
...
@members {
    ...
    void registerPrefix(String prefix, String uri) {
        registerPrefix(prefixMapping, prefix, uri);
    }

    void registerPrefix(PrefixMapping prefixMapping, String prefix, String uri) {
        // Warn if prefix gets re-registered
        String oldUri = prefixMapping.getNsPrefixURI(prefix);

        if(oldUri != null) {
            logger.warn("Prefix '" + prefix + "' remapped from <" + oldUri + "> to <" + uri + ">");
        }
...
viewDefinition returns [ ViewDefinition value ]
    : ^(VIEW_DEFINITION a=NAME b=viewTemplateDefinition d=varConstraints? c=sqlRelation?) { $value = ViewTemplateDefinition.createViewDefinition($a.text, $b.value, $c.value, $d.value); }
    ;
...
typeCtorExpression returns [Expr value]
    ...
    | ^(TYPED_LITERAL a=expression b=expression?) {$value = new E_StrDatatype($a.value, $b.value); }
    | ^(STRLANG a=expression b=expression) {$value = new E_StrLang($a.value, $b.value); }
    | ^(STRDT a=expression b=expression) {$value = new E_StrDatatype($a.value, $b.value); }
    ;

/*
typeCtorExpressionOld returns [Expr value]
...
sqlRelation returns [LogicalTable value]
//    : ^(RELATION_REF a=joinClause b=expression?) {$value=($b.value == null) ? a : new SqlOpFilterExpr(null, $a.value, $b.value);}
    : ^(RELATION_REF a=joinClause) {$value=a;}
    ;

joinClause returns [LogicalTable value]
    : a=joinClauseMember {$value=$a.value;}
    ;

joinClauseMember returns [LogicalTable value]
    : ^(JOIN_MEMBER a=sqlLogicalTable) {$value = $a.value;}
    ;

sqlLogicalTable returns [LogicalTable value]
    : ^(SQL_RELATION a=SQL_QUERY) {$value = new LogicalTableQueryString($a.text);}
    | ^(SQL_RELATION a=SQL_TABLE) {$value = new LogicalTableTableName($a.text);}
    ;
...
regexVarConstraint returns [ConstraintRegex value]
    : ^(REGEX_CONSTRAINT a=var b=string) {$value = new ConstraintRegex($a.value, "value", $b.value);}
    ;

prefixVarConstraint returns [ConstraintPrefix value]
    : ^(PREFIX_CONSTRAINT a=var b=stringList) {$value = new ConstraintPrefix($a.value, "value", $b.value);}
    ;
...
primaryExpression returns [Expr value]
    ...
    | e=expression        {$value = $e.value;} // For bracketted expressions
